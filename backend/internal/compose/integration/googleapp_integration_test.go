...
func TestRotatingTheGoogleAppRetiresTheOldSecret(t *testing.T) {
	...
	if _, err := searchVault(t, e).Get(ctx, ids.From[ids.WorkspaceKind](e.WS), keyvault.Ref(firstRef)); err == nil {
		t.Error("the superseded client secret is still readable from the vault")
	}
}
...
func TestRemovingTheGoogleAppIsIdempotentAndDestroysTheSecret(t *testing.T) {
	...
	if _, err := searchVault(t, e).Get(ctx, ids.From[ids.WorkspaceKind](e.WS), keyvault.Ref(ref)); err == nil {
		t.Error("the removed client secret is still readable from the vault")
	}
	if err := store.Remove(ctx); err != nil {
